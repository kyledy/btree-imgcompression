...
TEST_CASE("PTree::Constructor1", "[weight=1][part=ptree]") {
  cout << "Testing PTree constructor..." << endl;

  // manually create a PNG, 2x1
  PNG source;
  source.resize(2, 1);
  ...
  */
  ...
  REQUIRE(nd_root->upperleft == make_pair((unsigned int) 0, (unsigned int) 0));
  REQUIRE(nd_root->B->upperleft == make_pair((unsigned int) 1, (unsigned int) 0));

  REQUIRE(nd_root->A->A == nullptr);
}

  TEST_CASE("PTree::Constructor2", "[weight=1][part=ptree]") {
  cout << "Testing PTree constructor..." << endl;
...
TEST_CASE("PTree::Prune_large_render", "[weight=1][part=ptree]") {
  ...
  int treesize = tree.Size();
  int treenumleaves = tree.NumLeaves();
  // cout << treesize << " " << treenumleaves << endl;
...
TEST_CASE("PTree::Prune_FlipH_FlipV", "[weight=1][part=ptree]") {
  ...
  PNG output1 = tree.Render();
  output1.writeToFile("images/flip-horizontal-cs.png");
